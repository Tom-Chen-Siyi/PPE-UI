...
# Force real-time console output
sys.stdout.reconfigure(line_buffering=True)

def extract_frames(video_path, output_dir, every_n=1):
    """
    Extract frames from a video and save them as PNG images (OpenCV only).
    :param video_path: Path to the video file.
    :param output_dir: Directory to save extracted frames.
    :param every_n: Save every nth frame.
    :return: Total number of frames saved.
    """
    ...
    while True:
        ...
        if idx % every_n == 0:
            ...
            cv2.imwrite(fname, frame)  # directly save with cv2
            saved += 1
